...
export const INDICATOR_DESCRIPTIONS: Record<string, LocalizedDescription> = {
  ...
  buySellZone: {
    ...
    details: {
      RU: "Каждый компонент нормируется в 0..100 и усредняется по доступным с весами (по умолчанию LS 0.35, RSI 0.25, MACD 0.20, Bar 0.20, Net OI 0.20). Пятый компонент — Net OI Long/Short: перевес Net Short тянет линию вверх (зона продаж), перевес Net Long — вниз (зона покупок). Значения внутри коридора 35–65 — баланс; выше 80 — перегрев покупателей (красная заливка), ниже 20 — перегрев продавцов (зелёная). Если у свечи нет данных стакана/соотношения/OI или мало истории для z-score, компонент исключается, а вес перераспределяется на оставшиеся.",
      EN: "Each component is normalized to 0..100 and averaged over those available with weights (default LS 0.35, RSI 0.25, MACD 0.20, Bar 0.20, Net OI 0.20). The fifth component is Net OI Long/Short: Net Short dominance pulls the line up (sell zone), Net Long dominance pulls it down (buy zone). Values inside the 35–65 corridor mean balance; above 80 — buyer overheat (red fill), below 20 — seller overheat (green). If a candle lacks order-book/ratio/OI data or has too little history for a z-score, the component is dropped and its weight is redistributed to the rest.",
      KZ: "Әр компонент 0..100-ге қалыпқа келтіріледі және қолжетімділері салмақтармен орташаланады (әдепкі LS 0.35, RSI 0.25, MACD 0.20, Bar 0.20, Net OI 0.20). Бесінші компонент — Net OI Long/Short: Net Short басымдығы сызықты жоғары тартады (сату аймағы), Net Long басымдығы — төмен (сатып алу аймағы). 35–65 дәлізі ішіндегі мәндер — тепе-теңдік; 80-нен жоғары — сатып алушылардың қызуы (қызыл толтыру), 20-дан төмен — сатушылардың қызуы (жасыл). Егер свечада стакан/арақатынас/OI деректері болмаса немесе z-score үшін тарих аз болса, компонент алынып тасталып, оның салмағы қалғандарына қайта бөлінеді.",
    },
  },
  dynamicLevels: {
    desc: {
