...
- **`frontend/src/chart2d/ClusterChart.tsx`** — мемо `netOiPoints`: считаются `startIdx/endIdx` той же формулой, что автоскейл/рендер подвалов (`visibleScrollLeft`, `visibleClientWidth`, `candleWidth`, `candleSpacing`, `margin.left`), пробрасываются в `computeNetOiPoints`. deps дополнены `visibleScrollLeft`, `visibleClientWidth` → пересчёт при скролле/зуме (как longShortRatio / CVD visible). Автоскейл подвала уже читает пересчитанные точки — правок не потребовалось. Комментарий мемо обновлён.

**Verification:** `npx tsc --noEmit` ✓ (exit=0), `npx vite build` ✓ (exit=0, только предсуществующее предупреждение >500kB). Grep `netOiWindow|netWindow` по `frontend/src` — 0 совпадений (окно полностью удалено). Live-визуал не проверялся (бэкенд по условию не запускался): ожидаемо линии стартуют ~0 на левом крае видимого окна, расходятся умеренно (дефолт `smoothing=0.5`), не зеркальны, не схлопываются; при скролле окно ребейзится. Ползунок «Сглаживание»: выше → уже размах, ниже → шире. Ползунка «Окно» нет. Market/limit меняет доминанту. Визуальную приёмку делает пользователь.

**TODO:** тариф-гейтинг Net OI (пока доступен всем); визуальная приёмка на BTCUSDT FUTURES.

### [2026-07-01] fix(frontend): Net OI Long/Short — стабильная накопительная модель с фиксированной базой (убран Window)
Проблема: скользящее окно + min/max нормировка нестабильны — всплеск пересчитывал шкалу, ползунок «Окно» кардинально менял вид, значения неинтерпретируемы. Заменено на стабильную накопительную модель с ФИКСИРОВАННОЙ базой (OI первого бара с известным OI). Ползунок «Окно» и параметр `netOiWindow` удалены полностью. Бэкенд/OI-индикатор/знак дельты/цвета/выключение серий/market-limit своп/smoothing-усиление не трогали.
- **`frontend/src/chart2d/indicators/netOpenInterest.ts`** — `computeNetOiPoints(candles, oiClose, flowType, smoothing)`: убраны параметр `window`, кольцевой буфер, min-max нормировка (revert прошлой итерации). `base = OI` первого бара с известным OI (const на весь расчёт). `longCum/shortCum` копятся от начала истории по знаковым вкладам: доли потока с усилением `p=1/smoothing` + market/limit своп; `dOI≥0` → набор, `dOI<0` → кресто-своп (закрытие). `nlPct=longCum/base·100`, `nsPct=shortCum/base·100`. Серии НЕЗАВИСИМЫЕ (не зеркала, `nl+ns≠100`), стабильные (базa фиксирована → зум/скролл не пересчитывают шкалу), интерпретируемые (шорт вверх = шорты набирают, шорт вниз = закрывают); значения могут выходить за 0..100 — это норма. Бары без OI → null (разрыв, накопители/prevOi не трогаем). Doc обновлён. Дефолт `netOiSmoothing: 0.5` (умеренный размах). Убрано поле `netOiWindow` из `NetOpenInterestSettings`.
- **`frontend/src/chart2d/types.ts`** — убрано `IndicatorSettings.netOiWindow`.
- **`frontend/src/chart2d/ClusterChart.tsx`** — убран extract `netOiWindow`, убран из аргументов вызова и deps мемо `netOiPoints`. Fallback smoothing → 0.5. Комментарий мемо обновлён.
- **`frontend/src/components/IndicatorsModal.tsx`** — удалён блок ползунка «Окно», fallback smoothing UI → 0.5.
- **`frontend/src/i18n/dictionaries/{ru,en,kz}.ts`** — удалены ключи `netWindow`/`netWindowHint`.

**Verification:** `npx tsc --noEmit` ✓ (exit=0), `npx vite build` ✓ (exit=0, только предсуществующее предупреждение >500kB). Стабильность: `base` фиксирован после первого валидного OI → зум/скролл/подгрузка новых баров вправо не меняют шкалу. Live-визуал не проверялся (бэкенд по условию не запускался): ожидаемо линии стабильны при зуме/скролле; NL и NS не зеркальны, разной формы; ползунка «Окно» нет; «Сглаживание» меняет размах предсказуемо; market/limit — доминанту. Визуальную приёмку делает пользователь.

**TODO:** тариф-гейтинг Net OI (пока доступен всем); визуальная приёмка на BTCUSDT FUTURES.

### [2026-07-01] fix(frontend): Net OI Long/Short — две НЕЗАВИСИМЫЕ серии (не зеркала, nl+ns ≠ 100)
Проблема: прежняя модель считала долю одного целого → NL и NS были точным зеркалом (`ns=100−nl`). У эталона (mmt.gg) линии независимые (в сумме ≠ 100%). Модель заменена: знаковые вклады сторон в скользящем окне + НЕЗАВИСИМАЯ min-max нормировка каждой серии к 0..100. Сигнатура `computeNetOiPoints(candles, oiClose, flowType, smoothing, window)` не изменилась. Настройки (Window, Smoothing, show*, displayMode, flowType, цвета) не удалялись. Бэкенд/OI-индикатор/знак дельты/цвета/выключение серий/market-limit своп/smoothing-усиление не трогали.
- **`frontend/src/chart2d/indicators/netOpenInterest.ts`** — `computeNetOiPoints` перепроектирован в 2 прохода:
  - **Проход 1** (O(N), кольцевой буфер длиной W): пер бар с OI `dOI = oiNow−prevOi`; доли потока `buyFrac/sellFrac` с усилением `p=1/smoothing` + market/limit своп. Знаковые вклады: `dOI≥0` → `lc=dOI·buyFrac, sc=dOI·sellFrac`; `dOI<0` → кресто-своп (закрытие снимается с соответствующей стороны). Бегущие суммы `L_i=Σ_lastW lc`, `S_i=Σ_lastW sc` через `Float64Array`×2 (`L -= выпадающий + new`). Бары без OI: `lc=sc=0` (окно не рвём), но точка помечена «нет данных» → `null` в выходе.
  - **Проход 2**: `Lmin/Lmax` по всем валидным `L_i`, `Smin/Smax` — ОТДЕЛЬНО по `S_i`. `nlPct = 100·(L_i−Lmin)/Lspan` (или 50 если `Lspan=0`), `nsPct` аналогично. Разные диапазоны + разные знаковые суммы → линии НЕ зеркальны.
  - Бары без данных → `{nlPct:null, nsPct:null}` (разрыв).
- **`frontend/src/chart2d/ClusterChart.tsx`** — обновлён комментарий у мемо `netOiPoints` (две независимые серии, min-max по каждой). Сигнатура вызова та же, deps не менялись.

**Verification:** `npx tsc --noEmit` ✓ (exit=0), `npx vite build` ✓ (exit=0, только предсуществующее предупреждение >500kB). Первая попытка упала на `noUncheckedIndexedAccess` (индекс в `(number|null)[]` даёт `undefined`) → чтения обёрнуты в `?? null`, чисто. Производительность: O(N) (кольцевой буфер + два прохода), для N до нескольких тысяч мгновенно. Инвариант зеркальности УМЫШЛЕННО нарушен: `nl+ns` больше НЕ равно 100 — так и должно быть по ТЗ. Live-визуал не проверялся (бэкенд по условию не запускался): ожидаемо NL и NS разной формы, обе в 0–100%, свингуют, пересекаются; Window (короче → чаще/резче), Smoothing (размах долей), market/limit (доминанта) — влияют. Визуальную приёмку делает пользователь.

**TODO:** тариф-гейтинг Net OI (пока доступен всем); визуальная приёмка на BTCUSDT FUTURES.

### [2026-07-01] feat(frontend): Net OI Long/Short — модель «скользящее окно давлений» + настройка «Окно»
Проблема: комплементарная доля абсолютного OI — тупик, линии залипали зеркально у 50%. Смена модели на СКОЛЬЗЯЩЕЕ ОКНО потока: свингует 0–100% и пересекается на 50% при смене режима, mean-reverting (как эталон mmt.gg). Убрана прежняя абсолютно-комплементарная логика (longEst/shortEst=OI). Бэкенд/OI-индикатор/знак дельты/цвета/выключение серий/market-limit своп/smoothing-усиление не трогали.
- **`frontend/src/chart2d/indicators/netOpenInterest.ts`** — `computeNetOiPoints` перепроектирован. Пер бар: если есть OI и объём — `activity=|dOI|` делится по долям потока (степенное усиление `p=1/smoothing` + market/limit своп) на `longP/shortP`, иначе `longP=shortP=0` (окно не разрывается). Кольцевой буфер длины `W` (`Float64Array` × 2) + бегущие суммы `L,S` (`L += longP − выпадающий`) → O(N) на весь ряд, никаких O(N·W). Точка: `L+S>0` → `nlPct=L/(L+S)·100`, `nsPct=100−nlPct`; иначе null (разрыв). Убраны `longEst/shortEst`. Doc обновлён. Дефолт `netOiWindow=48` в `defaultSettings`, новое поле `netOiWindow?:number` в интерфейсе настроек.
- **`frontend/src/chart2d/types.ts`** — `IndicatorSettings.netOiWindow?:number`.
- **`frontend/src/chart2d/ClusterChart.tsx`** — извлечён `netOiWindow` (fallback 48), проброшен 5-м аргументом в `computeNetOiPoints`; добавлен в deps мемо `netOiPoints`.
- **`frontend/src/components/IndicatorsModal.tsx`** — ползунок «Окно» (range 5..500, шаг 1, дефолт 48) рядом со «Сглаживанием», с числовым индикатором и хинтом.
- **`frontend/src/i18n/dictionaries/{ru,en,kz}.ts`** — `netWindow`/`netWindowHint` (RU «Окно» / «меньше = чаще пересечения NL/NS», EN «Window» / «lower = more frequent NL/NS crossings», KZ «Терезе» / «аз = NL/NS қиылысулары жиі»).

**Verification:** `npx tsc --noEmit` ✓ (exit=0), `npx vite build` ✓ (exit=0, только предсуществующее предупреждение >500kB). Инвариант проверен вручную: сумма долей потока = 1 (в т.ч. после свопа) → `longP+shortP=activity`; `nlPct+nsPct=100` при `L+S>0`. Производительность: O(N) на пересчёт (кольцевой буфер, без O(N·W)). Live-визуал не проверялся (бэкенд по условию не запускался): ожидаемо NL/NS свингуют 0–100%, пересекаются на 50%, короткое «Окно» → чаще/резче пересечения, длинное → плавнее; «Сглаживание» → размах от 50%; market/limit меняет доминирующую сторону. Визуальную приёмку делает пользователь.

**TODO:** тариф-гейтинг Net OI (пока доступен всем); визуальная приёмка на BTCUSDT FUTURES.

### [2026-07-01] fix(frontend): Net OI Long/Short — нормировка как доли OI (сумма 100%, seed 50/50), откат окна
Проблема: NL/NS сильно расходились и не в сумме 100%, тогда как у эталона (mmt.gg) это ДОЛИ в сумме 100% (зеркальные, полоса 0–100%, пересекаются на 50%). Сменена нормировка на «оценка long-OI и short-OI, в сумме = текущий OI». Оконный (visible-range) расчёт прошлой итерации откачен — копим от начала загруженной истории. Smoothing-усиление, market/limit своп, знак дельты, бэкенд, OI-индикатор, режимы линия/свечи/цвета/выключение серий не трогали.
- **`frontend/src/chart2d/indicators/netOpenInterest.ts`** — `computeNetOiPoints` без `startIdx/endIdx` (revert окна). Накопители `longEst/shortEst` вместо `nlCum/nsCum`. Первый бар с OI (`oiNow>0`): сид `longEst=shortEst=oiNow/2`, `prevOi=oiNow`, точка `50/50`. Далее: `dOI=oiNow−prevOi`; доли потока (степенное усиление p=1/smoothing + market/limit своп) — как раньше; `dOI≥0` → `longEst+=dOI·buyFrac, shortEst+=dOI·sellFrac`, иначе крест. Инвариант `longEst+shortEst=oiNow`; защита от округления `longEst=clamp(0,oiNow); shortEst=oiNow−longEst`. `nlPct=longEst/oiNow·100`, `nsPct=shortEst/oiNow·100` (сумма 100). Бары без OI → null (накопители/prevOi не трогаем). Doc обновлён.
- **`frontend/src/chart2d/ClusterChart.tsx`** — мемо `netOiPoints`: убраны вычисление/передача `startIdx/endIdx` и deps `visibleScrollLeft/visibleClientWidth`. Вызов `computeNetOiPoints(candles, oiClose, flowType, smoothing)`; deps: candles/netOiOiMap/flowType/smoothing/candleWidth/candleSpacing/margin.left.

**Verification:** `npx tsc --noEmit` ✓ (exit=0), `npx vite build` ✓ (exit=0, только предсуществующее предупреждение >500kB). Инвариант проверен вручную: сумма `buyFrac+sellFrac=1` (в т.ч. после свопа) → после каждого бара `longEst+shortEst=prevOi+dOI=oiNow`; clamp удерживает 0..oiNow → `nlPct+nsPct=100`. Live-визуал не проверялся (бэкенд по условию не запускался): ожидаемо NL/NS зеркальны, в полосе 0–100%, пересекаются на 50%, не разъезжаются и не уходят в минус; smoothing регулирует размах от 50%, market/limit — какая сторона доминирует. Визуальную приёмку делает пользователь.

**TODO:** тариф-гейтинг Net OI (пока доступен всем); визуальная приёмка на BTCUSDT FUTURES.

### [2026-07-01] fix(frontend): Net OI Long/Short — накопление по видимому окну (rebase, как CVD "visible")
БАГ: копление шло от начала ВСЕЙ загруженной истории → монотонный дрейф, NL уползала вверх, NS вниз, уходили в минус. Переведено на накопление по ВИДИМОМУ диапазону: на левом крае экрана обнуляется, дрейф ограничен шириной окна. Механику усиления/smoothing, market/limit, знак дельты, бэкенд, OI-индикатор не трогали.
- **`frontend/src/chart2d/indicators/netOpenInterest.ts`** — `computeNetOiPoints` получил `startIdx`/`endIdx` (границы видимого окна). Цикл идёт по всем `candles` (индексы выхода совпадают с candles для отрисовки), но бары вне `[startIdx..endIdx]` → `nlPct/nsPct=null`. Накопители `nlCum/nsCum/prevOi` стартуют с нуля/null → на первом видимом баре с известным OI `dOI=0` (ребейз). Нормировка % от текущего OI и усиление — без изменений. Обновлён doc-комментарий.
- **`frontend/src/chart2d/ClusterChart.tsx`** — в мемо `netOiPoints` считаются `startIdx/endIdx` той же формулой, что автоскейл/рендер подвалов (`visibleScrollLeft`, `visibleClientWidth`, `candleWidth`, `candleSpacing`, `margin.left`), пробрасываются в `computeNetOiPoints`; `visibleScrollLeft`/`visibleClientWidth` добавлены в deps мемо → пересчёт на скролл/зум (как у CVD visible / longShortRatio). Автоскейл подвала уже читает пересчитанные (windowed) точки — правок не потребовалось.

**Verification:** `npx tsc --noEmit` ✓ (exit=0), `npx vite build` ✓ (exit=0, только предсуществующее предупреждение >500kB). Производительность: пересчёт по видимому окну на скролл — та же частота и путь, что у существующих подвалов (одна аллокация массива точек, как раньше). Live-визуал не проверялся (бэкенд по условию не запускался): ожидаемо линии больше не уползают за всю историю, слева стартуют ~0, расходятся вправо по набору лонгов/шортов, при скролле окно ребейзится. Визуальную приёмку делает пользователь.

**TODO:** тариф-гейтинг Net OI (пока доступен всем); визуальная приёмка на BTCUSDT FUTURES.

### [2026-07-01] fix(frontend): Net OI Long/Short — нелинейное усиление долей + настройка «Сглаживание»
БАГ: линии NL и NS почти сливались и повторяли форму OI. Причина — линейное деление прироста OI по долям потока (~50/50): разница линий = Σ dOI·(delta/V) микроскопическая, сумма задаётся формой OI → обе ≈ OI/2. Референс (mmt.gg) применяет нелинейное усиление перевеса с параметром «Quadratic Smoothing». Добавлено усиление долей степенью p=1/smoothing + настройка. Инвариант сохранён: `longAdd+shortAdd = dOI` на каждом баре (доли по-прежнему в сумме = 1). Бэкенд/OI-индикатор/знак дельты не тронуты.

- **`frontend/src/chart2d/indicators/netOpenInterest.ts`** — `computeNetOiPoints` получил параметр `smoothing:number`. Вместо линейных долей: `p = 1/clamp(smoothing,0.05,1)`; `bw=pow(takerBuy,p)`, `sw=pow(takerSell,p)`; `buyFracTaker = bw+sw>0 ? bw/(bw+sw) : 0.5`; `sellFracTaker=1−buyFracTaker`. `takerBuy/Sell` через `Math.max(0,…)` (guard от NaN у `pow(neg, дробн.)`). Market/limit-своп, направленное деление dOI, накопление, `nlPct/nsPct` — без изменений. Обновлён doc-комментарий. `defaultSettings.netOiSmoothing = 0.3`, в `NetOpenInterestSettings` добавлен `netOiSmoothing?:number`.
- **`frontend/src/chart2d/types.ts`** — `IndicatorSettings.netOiSmoothing?:number`.
- **`frontend/src/chart2d/ClusterChart.tsx`** — извлечение `netOiSmoothing` (fallback 0.3), проброс 4-м аргументом в `computeNetOiPoints`, добавлен в deps мемо `netOiPoints`.
- **`frontend/src/components/IndicatorsModal.tsx`** — контрол «Сглаживание»: `range` 0.05..1 шаг 0.05, дефолт 0.3, значение `.toFixed(2)`, хинт под ползунком.
- **`frontend/src/i18n/dictionaries/{ru,en,kz}.ts`** — `netSmoothing`, `netSmoothingHint` («меньше = резче расхождение NL/NS»).

**Verification:** `npx tsc --noEmit` ✓ (exit=0), `npx vite build` ✓ (exit=0, только предсуществующее предупреждение >500kB). Матем-инвариант проверен вручную: `buyFrac+sellFrac=1` при любом smoothing (в т.ч. после свопа market/limit) → `longAdd+shortAdd=dOI`. Live-визуал не проверялся (бэкенд по условию не запускался): ожидаемо на BTCUSDT FUTURES NL/NS расходятся и пересекаются; ползунок 1.0 → линии сходятся (прежнее поведение), 0.05 → резкие расхождения. Визуальную приёмку делает пользователь.

**TODO:** тариф-гейтинг Net OI (пока доступен всем); визуальная приёмка на BTCUSDT FUTURES.

### [2026-07-01] feat(frontend): индикатор Net OI Long/Short — подвал, 2 серии (NL/NS), линия/свечи
Новый подвальный индикатор оценки позиционирования по открытому интересу. ОДНА панель, ДВЕ серии: Net Long (NL) и Net Short (NS) в % от текущего OI. Чисто фронтовая фича: считается из уже загруженных данных (OI-ряд + поток сделок по свечам), БЕЗ правок бэкенда. Переиспользован `fetchOpenInterest` (индикатор Open Interest / Фаза C НЕ тронут). Структура подвала — по образцу openInterest, свечи — по образцу CVD. Тариф-гейта нет.

**Алгоритм** (по свечам по времени, futures-only; `candle.delta` уже нормализована как BUY−SELL — второй минус НЕ добавляли): `takerBuy=(V+delta)/2`, `takerSell=(V−delta)/2`; при `flowType=market` доли по тейкеру, при `limit` — по мейкеру (свап); `dOI=OI_i.c−OI_{i-1}.c` (для первого известного OI dOI=0); при `dOI≥0` longAdd=dOI·buyFrac, shortAdd=dOI·sellFrac, иначе крест; накопление NL_cum/NS_cum от начала загруженной истории; значения `NL%=NL_cum/OI_now·100`, `NS%` аналогично. Точки `{t,nlPct,nsPct}` (null если нет OI для бара).

- **`frontend/src/chart2d/indicators/netOpenInterest.ts`** (новый) — модуль id `netOpenInterest`, type `Подвальный`, label `(PROCLUSTER) Net OI Long/Short`, `isActiveDefault:false`, defaultSettings `{ netOiShowLong:true, netOiShowShort:true, netOiDisplayMode:"candles", netOiFlowType:"limit", netOiLongColor:"#26a69a", netOiShortColor:"#ef5350" }`. Экспортирует чистую `computeNetOiPoints(candles, oiClose, flowType)`.
- **`frontend/src/chart2d/indicators/index.ts`** — import/export + в `MODULAR_INDICATORS` (после openInterest). Доп-мёржится существующим юзерам через `storage.ts` с дефолтами.
- **`frontend/src/chart2d/types.ts`** — в `IndicatorSettings`: `netOiShowLong/ShowShort?:boolean`, `netOiDisplayMode?:"line"|"candles"`, `netOiFlowType?:"market"|"limit"`, `netOiLongColor?/ShortColor?:string`.
- **`frontend/src/chart2d/ClusterChart.tsx`** — полный набор точек интеграции подвала (зеркалит openInterest, но с двумя сериями): своя fetch-стейт `netOiData` (переиспользует `fetchOpenInterest`, независим от индикатора OI), `netOiOiMap`, `netOiPoints` (cx + nl/ns через `computeNetOiPoints`), авто-скейл видимого диапазона по ОБЕИМ включённым сериям, высота/persist панели, `netOiTopY`, scale/offset state + drag (мышь+тач), ветка resize. Отрисовка: **line** — две линии NL/NS (свои цвета, разрыв `moveTo` на null); **candles** — по каждой включённой серии свечи как у CVD (open=пред. значение серии, close=текущее, тело в цвет серии, затемнение при close<open через alpha). Нулевая пунктирная линия 0%. Выключенная серия не рисуется и не учитывается в авто-скейле. Y-подписи в % (`fmtNetOiPct`). Легенда-плашка: два чипа «NL %»/«NS %» с цветными точками (чип выключенной серии скрыт). На споте — «Только futures».
- **`frontend/src/components/IndicatorsModal.tsx`** — блок настроек `netOpenInterest`: 2 чекбокса Show Long/Show Short, селектор режима (Линия/Свечи), селектор потока (Market/Limit), 2 color picker (цвет Long/Short).
- **`frontend/src/chart2d/indicators/descriptions.ts`** — локализованные desc/details (RU/EN/KZ).
- **`frontend/src/i18n/dictionaries/{ru,en,kz}.ts`** — `netParams`, `netShowLong`, `netShowShort`, `netFlow`, `netFlowMarket`, `netFlowLimit`, `netLongColor`, `netShortColor` (переиспользуются `mode`, `oiLine`, `oiCandles`).

**Verification:** `npx tsc --noEmit` ✓ (exit=0, без ошибок), `npx vite build` ✓ (built, только предсуществующее предупреждение о размере чанка >500kB). Live-визуал в браузере НЕ проверялся: для данных нужен запущенный Go-бэкенд (`/api/v1/open-interest`) и futures-свечи, а по условию задачи бэкенд не запускался — визуальную проверку (npm run dev, BTCUSDT FUTURES: две серии NL/NS в %; выключение каждой серии; линия↔свечи; Market↔Limit меняет картину; смена цветов; пусто на SPOT; пересчёт при скролле истории) делает пользователь.

**TODO:** тариф-гейтинг Net OI (пока доступен всем); визуальная приёмка пользователем на BTCUSDT FUTURES.
...
### [2026-06-30] feat(frontend): объекты рисования — undo (Ctrl+Z), рисование в 2 клика, кнопка-замок
Три улучшения объектов рисования графика (Canvas2D, src/chart2d/). Движок не переписан — точечные правки.
- **`frontend/src/chart2d/ClusterChart.tsx`**:
  - **Undo (Ctrl+Z)** с нуля: `drawingsRef` + `undoStackRef` (кап 50, `structuredClone`), хелпер `pushUndo()`. Снапшот ДО каждого изменения списка: создание horizontal/нового объекта/канала, коммит текста из модалки, старт drag/resize (1 снапшот на жест), Delete, clear-all. Ctrl/Cmd+Z в keydown-эффекте: pop стека → `setDrawings(snap)`. Per-frame апдейтеры drag/preview снапшот НЕ пушат.
  - **Рисование в 2 клика**: финализация перенесена с mouseup на ВТОРОЙ mousedown (mousemove уже двигает превью без зажатой кнопки). В `handleMouseDown` блок `if (drawingInProgress)`: канал stage2→завершить, stage1→stage2, text→модалка, остальные→push в drawings. Блок финализации в `handleMouseUpOrLeave` удалён (остался только сброс drag/пана). **Esc** в keydown отменяет рисование (`drawingInProgress`→null).
  - **Замок**: state `drawingsLocked` + персист в localStorage `procluster_drawings_locked` (по образцу `areDrawingsVisible`). Блокирует ТОЛЬКО перемещение/ресайз/удаление существующих — рисовать новые можно. Гарды: вход в блок выделения/drag (`&& !drawingsLocked`), Delete (`&& !drawingsLockedRef.current`), clear-all (ранний return). Клик по объекту при замке проходит насквозь (панорама).
- **`frontend/src/chart2d/DrawingToolbar.tsx`**: кнопка-тоггл Lock/LockOpen (lucide) между Eye и Trash, стиль зеркалит кнопку видимости (amber при активном), подсказка RU «Заблокировать объекты» / EN «Lock drawings». Trash (clear-all) скрыт при замке.

**Verification:** `npx tsc --noEmit` ✓ (TSC_EXIT=0), `npx vite build` ✓ (built in 661ms, VITE_EXIT=0; только предсуществующие предупреждения — порядок CSS @import и размер чанка >500kB).

**TODO:** нет.

### [2026-06-30] fix(frontend/mobile): мобильная панель «Настройки графика» = overlay вместо push-down
На мобиле панель настроек была сиблингом между `<header>` и областью графика внутри корневого `flex flex-col`, с классом `relative` → в потоке → при открытии толкала график вниз.
- **`frontend/src/App.tsx`**: блок мобильной панели (`AnimatePresence` + `motion.div`) перенесён ВНУТРЬ `<header>` (хедер уже `relative z-[1100]` — позиционирующий предок). В className панели `relative` → `absolute top-full left-0 right-0`: панель выпадает ровно под хедер, поверх графика, вне потока. Анимация (height+opacity), `lg:hidden`, glass/blur, `max-h-[85vh] overflow-y-auto` — без изменений. Десктоп не тронут (панель `lg:hidden`).

**Verification:** `npx tsc --noEmit` ✓ (TSC_EXIT=0), `npx vite build` ✓ (built in 680ms, VITE_EXIT=0; предсуществующие предупреждения). Playwright, вьюпорт 390×844: скрин до/после открытия «Настроек» — график на месте (ценовой тег $58 639, линия $58 000, панель Bid & Ask, таймлайн 23:45 на тех же Y-координатах), панель выводится поверх. Консольная 401 (auth/refresh, гость) к вёрстке не относится.

**TODO:** нет.

### [2026-07-01] feat(backend): Open Interest — хранение + live-поллер + REST (Фаза A плана OI)
Индикатор Open Interest (открытый интерес фьючерсов) по образцу long/short ratio. Фронт и загрузчик истории НЕ тронуты (Фазы B/C). Храним два числа: `sum_open_interest` (контракты) и `sum_open_interest_value` (USD).
- **`backend/internal/repository/clickhouse/migrations/010_open_interest.sql`** (новый) — таблица `open_interest` (LowCardinality symbol/market, ts DateTime64(3), два Decimal(18,1)), `ReplacingMergeTree`, `PARTITION BY toYYYYMM(ts)`, `ORDER BY (symbol, market, ts)`, `TTL toDateTime(ts) + INTERVAL 1 YEAR` (безопасно, без переполнения DateTime).
- **`backend/internal/model/model.go`** — тип `OpenInterest{ Symbol, Market, TS, SumOpenInterest, SumOpenInterestValue }`.
- **`backend/internal/repository/repository.go`** — в интерфейс `MarketRepository` добавлены `InsertOpenInterestBatch` и `GetOpenInterest`.
- **`backend/internal/repository/clickhouse/clickhouse.go`** — реализация обоих методов по образцу LongShortRatio (`decimal.NewFromFloat` при Append, скан через `decimal.Decimal`).
- **`backend/internal/openinterest/poller.go`** (новый) — live-поллер, интервал 5 мин, только futures-символы, `GET fapi.binance.com/futures/data/openInterestHist?period=5m&limit=30` (бэкфилл ~2.5ч), значения из строк в float, пишет `InsertOpenInterestBatch`. Ошибки сети логируются и не рвут цикл. Первичный pollAll на старте.
- **`backend/internal/api/openinterest.go`** (новый) — `GET /api/v1/open-interest?symbol&market&timeframe&from&to`. Валидация + тариф-лимит истории (`resolveHistoryDepth`) как в эталоне. Группировка 5-мин точек по бакету таймфрейма в OHLC значения `sum_open_interest` (o=первое, h=max, l=min, c=последнее — OI это уровень, не суммируется). Ответ `[{t,o,h,l,c}]`.
- **`backend/internal/api/server.go`** — маршрут `/api/v1/open-interest` с теми же middleware (rate-limit + auth + beta-gate), что у long/short.
- **`backend/cmd/procluster/main.go`** — горутина `oiPoller` после `lsrPoller`, лог `[openinterest] started`.

**Verification:** `go build ./...` ✓ (BUILD_OK), `go vet` ✓ (VET_OK), `go test ./internal/api/ ./internal/repository/...` ✓ (все зелёные). Запущен backend (стек redis+clickhouse через docker-compose): в логах `[clickhouse] migrations applied` + `[openinterest] started`, ошибок миграции нет. Таблица `procluster.open_interest` наполняется — 182 строки (3 futures-символа: BTC/ETH/SOL по ~60, спот исключён корректно), хранятся оба Decimal. `curl "/api/v1/open-interest?symbol=BTCUSDT&market=futures&timeframe=1h&from=<now-1d>&to=<now>"` → непустой OHLC-массив, напр. `{"t":1782914400000,"o":106050.2,"h":106539.5,"l":106050.2,"c":106289.3}` (h≥o/c, l≤o/c — математика верна). procluster.exe остановлен (юзер запустит сам); docker-стек оставлен поднятым.

**TODO:** Фаза B — загрузчик истории OI (бэкфилл глубже 2.5ч). Фаза C — фронт (отрисовка индикатора, отображаем контракты).

### [2026-07-01] feat(backend+admin): Open Interest — история через history-loader (Фаза B плана OI)
Скачивание истории Open Interest в админ-загрузчике по образцу longShortRatio. Тот же metrics-CSV с data.binance.vision (колонки `sum_open_interest` / `sum_open_interest_value` уже есть). Фаза A и фронт-индикатор (Фаза C) не тронуты.
- **`backend/internal/admin/handlers.go`** (handleStartDownload) — `"openInterest"` добавлен в список допустимых dataType; гейт `market == "futures"` для openInterest (как у bookDepth/longShortRatio).
- **`backend/internal/admin/historyloader.go`**:
  - Интерфейс `HistoryClickHouse` — добавлен `InsertOpenInterestBatch(ctx, []model.OpenInterest) error`.
  - Ветка в `downloadWorker`: `if job.DataType == "openInterest"` → `downloadWorkerOpenInterest`.
  - Новый `downloadWorkerOpenInterest` — копия `downloadWorkerLongShort`: тот же `buildMetricsURL` (metrics ZIP), шаги downloading→parsing→inserting, без DELETE (ReplacingMergeTree идемпотентен по `symbol+market+ts`).
  - Новые `unzipAndParseMetricsOpenInterest` + `parseOpenInterestCSV` — колонки по имени case-insensitive (`create_time`, `sum_open_interest`, `sum_open_interest_value`); timestamp через `parseBookDepthTimestamp` (epoch ms/s или `YYYY-MM-DD HH:MM:SS`); строка пропускается если значения пустые/непарсятся/≤0; `Market="futures"`.
- **`backend/internal/admin/historyloader_test.go`** — в mockClickHouse добавлен `InsertOpenInterestBatch` (интерфейс расширен).
- **`frontend/src/components/AdminPanel.tsx`** — в форме загрузки: пункт dataType `openInterest` («Open Interest»), futures-pin в onChange, futures-only в списке market; бейдж `openInterest` (cyan, «Open Interest») в `dataTypeBadge`.

**Verification:** `go build ./...` ✓ (BUILD_OK), `go vet ./internal/admin/` ✓ (VET_OK), `go test ./internal/admin/...` ✓ (ok, 9.4s). Фронт: `npx tsc --noEmit` ✓ (TSC_OK), `npx vite build` ✓ (built, только предсуществующее предупреждение о размере чанка). Runtime: пересобран procluster.exe, запущен (стек redis+clickhouse). Через админ-API (`POST /api/v1/admin/history/download`, токен админа) запущена job `openInterest BTCUSDT futures 2026-06-27…06-29` → **status=completed**, «All 3 days processed successfully», totalTicks=864. `open_interest` BTCUSDT в диапазоне: baseline 0 → **863** уникальных точек (3×288, один дубль на границе суток схлопнут ReplacingMergeTree). Повторный запуск того же диапазона → distinct ts = 863, после `OPTIMIZE … FINAL` count = 863 — **дублей нет** (идемпотентно). procluster.exe остановлен, не оставлен запущенным.

**TODO:** Фаза C — фронт: отрисовка индикатора Open Interest (отображаем контракты `sum_open_interest`; данные из `GET /api/v1/open-interest`).

### [2026-07-01] feat(frontend): индикатор Open Interest — подвал, линия/свечи (Фаза C плана OI)
Подвальный индикатор Open Interest по образцу Long/Short Ratio (backend-fetch) + CVD (режим свечей). Значение = контракты (`sum_open_interest`, close/OHLC эндпоинта). Стиль: переключатель линия/свечи + цвет линии. futures-only, БЕЗ тариф-гейта. Бэкенд/history-loader не тронуты.
- **`frontend/src/features/openinterest/api.ts`** (новый) — `fetchOpenInterest(...)` → `OpenInterestPoint[]` (`{t,o,h,l,c}`), возврат `[]` при любой ошибке (не кидает в render loop). Копия `features/longshort/api.ts`.
- **`frontend/src/chart2d/indicators/openInterest.ts`** (новый) — модуль id `openInterest`, type `Подвальный`, label `(PROCLUSTER) Open Interest`, defaultSettings `{ openInterestDisplayMode: "line", openInterestLineColor: "#f59e0b" }`, isActiveDefault:false.
- **`frontend/src/chart2d/indicators/index.ts`** — import/export + в `MODULAR_INDICATORS` (после longShortRatio). Через `storage.ts` индикатор доп-мёржится существующим юзерам с дефолтами.
- **`frontend/src/chart2d/types.ts`** — в `IndicatorSettings`: `openInterestDisplayMode?: "line"|"candles"`, `openInterestLineColor?: string`.
- **`frontend/src/chart2d/ClusterChart.tsx`** — полный набор точек интеграции подвала (зеркалит longShortRatio): fetch-эффект (futures-only, deps `[active, symbol, timeframe, candleMinTs, candleMaxTs, accessToken]`), `openInterestMap`/`openInterestPoints` (cx центра + o/h/l/c по timestamp), авто-скейл видимого диапазона (candles→low/high, line→close), высота/persist панели, `openInterestTopY` в `panelTopY`, scale/offset state + drag (мышь+тач), ветка resize, отрисовка: **line** — сплошная линия по close (цвет `openInterestLineColor`, glow как у long/short); **candles** — OHLC-свечи (зелёная close≥open, красная close<open, как CVD); Y-подписи (компактный формат k/M/B через `fmtOi`), легенда-плашка, ручка ресайза, зоны mouse/touch. На споте — «Только futures».
- **`frontend/src/components/IndicatorsModal.tsx`** — блок настроек `openInterest`: селектор режима (Линия/Свечи) + color picker линии.
- **`frontend/src/chart2d/indicators/descriptions.ts`** — локализованные desc/details (RU/EN/KZ).
- **`frontend/src/i18n/dictionaries/{ru,en,kz}.ts`** — `oiParams` (заголовок), `oiLine`, `oiCandles` (переиспользуются `mode`, `lineColor`).

**Verification:** `npx tsc --noEmit` ✓ (TSC_DONE, без ошибок), `npx vite build` ✓ (built in 719ms, только предсуществующее предупреждение о размере чанка >500kB). Плюмбинг проверен по коду: `computeActiveIndicators` строит `activeIndicators` по `ind.id` (openInterest включается автоматически), `storage.ts` мёржит новый индикатор из `MODULAR_INDICATORS` с defaultSettings. Live-визуал в браузере НЕ проверялся: для данных нужен запущенный Go-бэкенд (`/api/v1/open-interest`), а по условию задачи бэкенд не запускался — визуальную проверку (npm run dev на BTCUSDT FUTURES: линия↔свечи, цвет, пусто на SPOT, подтяжка при скролле) делает пользователь.

**TODO:** Тариф-гейтинг индикатора Open Interest (пока доступен всем — как решит юзер).

### [2026-07-01] fix(admin): Open Interest в покрытии данных (coverage + gaps)
Блок «Покрытие данных» в админке не показывал Open Interest. Добавлен пятый источник покрытия. Фронт уже был готов (dataTypeBadge + таблица рендерят строки generically).
- **`backend/internal/repository/clickhouse/clickhouse.go`**:
  - `GetHistoryCoverage` — в slice `sources` добавлен `{table: "open_interest", timeCol: "ts", dataType: "openInterest", hasMarket: true}` (market из колонки, как у bookdepth/long_short). Комментарий функции: «по 4 источникам» → «по 5» (+open_interest).
  - `GetCoverageGaps` — в switch добавлен `case "openInterest"` (table `open_interest`, timeCol `ts`, where `symbol = ? AND market = ?`).
- **`backend/internal/admin/handlers.go`** (`handleCoverageGaps`, ~стр. 616) — в whitelist dataType добавлен `openInterest` (иначе gaps-эндпоинт резал запрос до repo с `INVALID_DATATYPE`). Не было в исходном ТЗ, но без этого gaps по openInterest не работает. Имена таблиц/колонок — константы кода; symbol/market идут только через параметры (?).

**Verification:** `go build ./...` ✓ (BUILD_OK), `go vet ./internal/repository/...` ✓ (VET_OK). Runtime (стек redis+clickhouse, `open_interest` = 9341 строк): admin `GET /api/v1/admin/history/coverage` → строка **openInterest BTCUSDT/futures: firstDay 2026-06-01, lastDay 2026-07-01, daysWithData 31, spanDays 31, missingDays 0** (плюс ETH/SOL по 1 дню от live-поллера). `GET /coverage/gaps?dataType=openInterest&symbol=BTCUSDT&market=futures` → `{"ok":true,"data":[]}` (без ошибки, пропусков нет). procluster.exe остановлен, не оставлен запущенным.

**TODO:** нет.

### [2026-07-02] fix(chart): бесшовная дозагрузка истории — прыжок вправо, «не всегда грузит», 1-кадровый перескок
Три бага дозагрузки старой истории при скролле влево (заметны на проде из-за сетевой латенси, на локалке почти нет). Причины подтверждены покадровым трейсом (chrome-devtools, Slow 3G). Всё в одном файле, backend не тронут.
- **`frontend/src/chart2d/ClusterChart.tsx`** — 3 связанных фикса:
  - **Прыжок вправо (клампа):** `prependScrollRef` синхронно ращу ширину спейсера `#procluster-chart-spacer` на `addedWidth` ДО сдвига `scrollLeft` (зеркалю приём из ctrl-wheel зума). Без этого спейсер ещё старой ширины → браузер клампит scrollLeft → живая компенсация молча падала и побеждал stale-anchor fallback (прыжок = дистанция скролла за время фетча ∝ латенси).
  - **«Не всегда грузит»:** триггер `onNeedHistory` перенесён в `onScroll` от ЖИВОГО scrollLeft (был на задебаунсенном state `visibleScrollLeft`, флаш только <200px или через 100мс после остановки → при непрерывном/инерционном скролле не срабатывал). Условие `idxF < 100` без старого `>= 0` — на самом краю (scrollLeft≈0) idxF отрицателен, старый гард глушил загрузку. Якорь пишется из живой позиции.
  - **1-кадровый перескок содержимого (атомарность):** новые рефы `committedCandlesRef` (зеркало закоммиченного массива, обновляется в draw-эффекте) + `prependPendingArrayRef`. `prependScrollRef` двигает scrollLeft до коммита `setCandles`, а scroll-event-driven draw в этом окне рисует СТАРЫЕ свечи при НОВОМ scrollLeft → контент уезжает на addedWidth на 1-2 кадра. Гейт в начале `drawRef`: пока `candles === prependPendingArrayRef.current`, `return` (не рисуем стале-массив, sticky-канвас держит последний хороший кадр). Любой новый коммит (prepend/смена пары-ТФ/live) открывает гейт — фриза «навсегда» нет. Без flushSync (дал бы фриз ~160мс), без синхронного draw (нарисовал бы ровно плохой кадр).

**Verification:** `npx tsc --noEmit` ✓ (TSC_OK), `npx vite build` ✓ (built, только предсуществующее предупреждение о чанке >500kB). Покадровый трейс (chrome-devtools, Slow 3G, зум ~87px/свеча): плохих кадров (старый `len` при новом `sl`) было 2 → стало **0**; первый кадр с новым scrollLeft уже с новым числом свечей (атомарно). Прыжок вправо устранён, дозагрузка у левого края (в т.ч. в упор) срабатывает. Не сломаны: зум ctrl/shift-wheel, reset, авто-fit, маунт, смена пары/ТФ (гейт неактивен вне prepend). Визуальную проверку на проде делает пользователь.

**TODO:** нет.

### [2026-07-02] feat(frontend): модалка тарифов по клику на закрытую функцию (tier-gating UI)
Клик по закрытой на тарифе опции → модальное окно с карточками тарифов (те же, что в профиле). Замки остаются. Фронт-only, backend не тронут (лимиты уже в `tier_policies`, отдаются через `/api/v1/user/limits` и `/api/v1/tiers`; enforcement на бэке — это только UI-подсказка).
- **`frontend/src/components/plans/PlanCards.tsx`** (новый) — вынос `PlanCard` + `LimitRow` + `formatHistoryValue` из UserProfile 1-в-1 (переиспользуемый экспорт).
- **`frontend/src/components/plans/PlansModal.tsx`** (новый) — модалка: `createPortal`→body, `z-[100000]` (выше IndicatorsModal z-1200 и дропдаунов 99999), liquid-glass + backdrop-blur, закрытие фон/Esc/крестик, `useTiers()` + 3 карточки free/pro/vip, подсветка текущего `limits.tier`, спиннер/тихо-пусто как в профиле, «Активировать» = alert (оплата не подключена).
- **`frontend/src/contexts/UpgradeModalContext.tsx`** (новый) — `UpgradeModalProvider` (state + рендер `<PlansModal/>`), хук `useUpgradeModal() → { openPlans }`.
- **`frontend/src/components/UserProfile.tsx`** — импорт `PlanCard` из plans/, удалён вынесенный код + неиспользуемый `PublicTierPolicy`. Профиль 1-в-1.
- **`frontend/src/App.tsx`** — `<UpgradeModalProvider>` внутри `LimitsProvider` (оборачивает AppShell); хук в AppShell; моб. аномалии → замок кликабелен (`disabled` убран), клик → `openPlans()`.
- **`frontend/src/components/ChartHeader.tsx`** — десктоп аномалии + уровни сжатия: `disabled` убран, лок/тултип/Lock-иконка остались, клик по закрытому → `openPlans()`.
- **`frontend/src/chart2d/ChartToolsHeader.tsx`** — layout «2 графика» isLocked → `openPlans()` (было `return`), 🔒 остался.
- **`frontend/src/components/IndicatorsModal.tsx`** — `toggleActive` при `activeCount>=maxIndicators`: тост `limitReached` заменён на `openPlans()` (индикатор не добавляется). Единственная точка стопа лимита; tierBlocked/gated/custom-settings 403 не тронуты.
- **`frontend/src/i18n/dictionaries/{ru,en,kz}.ts`** — блок `plans.modalTitle`/`plans.modalSubtitle` (3 языка); строки карточек переиспользуют `profile.*`.

**Verification:** `npx tsc --noEmit` ✓ (чисто), `npx vite build` ✓ (built, только предсуществующее предупреждение о чанке >500kB). Визуально проверено пользователем: под free клик по аномалиям (десктоп+моб)/layout 2 графика/уровню сжатия 2+/добавлению индикатора сверх лимита → модалка тарифов поверх (в т.ч. поверх IndicatorsModal); под pro/vip замков нет; профиль не сломан; темы dark/light, языки RU/EN/KZ.

**Вне охвата (не трогали):** гейтные индикаторы (buySellZone, скрытие из каталога), 403 CUSTOM_SETTINGS_FORBIDDEN (кастом-настройки) — их тосты как были.

**TODO:** подключить оплату к кнопке «Активировать» (пока alert «скоро»).

### [2026-07-02] fix(tiers): цены Pro/VIP серверные (было — localStorage браузера)
Баг: цены тарифов хранились только в localStorage браузера админа → у каждого свои, всем остальным показывался хардкод-fallback 19/49 вместо заданных в админке. Перенесено на сервер: колонка `price` в `tier_policies`, единый источник, отдаётся через `/api/v1/tiers` и `/api/v1/user/limits`.
- **`backend/internal/auth/sqlite.go`** — идемпотентная миграция add-column `{"price","INTEGER NOT NULL DEFAULT 0"}` (PRAGMA-проверка, не падает при повторном старте; старые строки → 0).
- **`backend/internal/admin/tier_policies.go`** — `Price int` в `TierPolicy` и `PublicTierPolicy`; сиды (guest/free/admin 0, pro 20, vip 40 — плейсхолдер). `price` добавлен во ВСЕ SQL: SeedTierPolicies, GetLimitsForTier, GetPolicies, GetPublicPolicies, UpsertPolicies (INSERT + ON CONFLICT price=excluded.price). Колонки=плейсхолдеры=аргументы выверены.
- **`backend/internal/admin/handlers.go`** — валидация price 0..100000 в handleUpdatePolicies.
- **`backend/internal/auth/handlers.go`** — `/user/limits`: price в struct (осн.+fallback) + SELECT + Scan (согласованы).
- **`backend/internal/admin/tier_policies_test.go`** — проверка сидов price (pro 20, vip 40).
- **`frontend/src/features/auth/api.ts`** — `price: number` в UserLimits (PublicTierPolicy получает через Omit).
- **`frontend/src/contexts/LimitsContext.tsx`** — `price: 0` в DEFAULT_LIMITS.
- **`frontend/src/features/admin/api.ts`** — `price` в админ-типе TierPolicy.
- **`frontend/src/components/plans/PlansModal.tsx`**, **`UserProfile.tsx`** — цена из `tiers.pro.price`/`tiers.vip.price` (free $0); localStorage + fallback 19/49 убраны; пока грузится — спиннер/пусто.
- **`frontend/src/components/AdminPanel.tsx`** — цены грузятся из `apiGetPolicies()`; «Сохранить цены» = GET policies → выставить price pro/vip → PUT назад (мерж, лимиты не трогает); localStorage.setItem убран.

**Verification:** `go build ./...` ✓, `go test ./internal/admin/` ✓ (вкл. price-сиды). `npx tsc --noEmit` ✓, `npx vite build` ✓. Бэк перезапущен: миграции применились идемпотентно (`already seeded, skipping`), без ошибок; `GET /api/v1/tiers` отдаёт `price`. На текущей dev-БД price=0 у всех (колонка с DEFAULT 0) — норма, админ выставляет реальные и они глобальны. Проверено пользователем локально.

**TODO:** подключить оплату к кнопке «Активировать» (пока alert «скоро»). После деплоя админу один раз выставить цены Pro/VIP в админке.
