// Package longshort содержит live-поллер глобального long/short account ratio
// фьючерсов Binance. Тянет данные с публичного futures-data эндпоинта на
...
const (
	binanceLSRURL = "https://fapi.binance.com/futures/data/globalLongShortAccountRatio"
	pollPeriod    = "5m"
...
// binanceLSREntry — один элемент JSON-ответа globalLongShortAccountRatio.
// longShortRatio приходит строкой; берём только его и timestamp (ms).
...
// Poller периодически опрашивает Binance и пишет точки в long_short_ratio.
type Poller struct {
...
func (p *Poller) pollAll(ctx context.Context) {
	for _, symbol := range p.symbols {
		...
		if err := p.pollSymbol(ctx, symbol); err != nil {
			log.Printf("[longshort] poll %s error: %v", symbol, err)
		}
	}
}
...
func (p *Poller) pollSymbol(ctx context.Context, symbol string) error {
	...
	rows := make([]model.LongShortRatio, 0, len(entries))
	for _, e := range entries {
		ratio, err := strconv.ParseFloat(e.LongShortRatio, 64)
		if err != nil {
			log.Printf("[longshort] bad ratio %q for %s: %v", e.LongShortRatio, symbol, err)
			continue
		}
	...
	if err := p.repo.InsertLongShortRatioBatch(ctx, rows); err != nil {
