...
type SMTPEmailSender struct {
	...
	Password   string
	From       string
	AppBaseURL string // base URL сайта, нужен для абсолютного src логотипа в письме
}
...
func (s *SMTPEmailSender) SendVerification(ctx context.Context, to, verifyURL string) error {
	...
	msg := buildVerificationMessage(from, to, verifyURL, s.AppBaseURL)
...
// verifyEmailHTMLTemplate — HTML письма подтверждения email в стиле PROCLUSTER.
// Email-safe: table-layout, inline-стили, bulletproof VML-кнопка для Outlook,
// preheader, web-safe шрифты. Плейсхолдеры {{VERIFY_URL}} / {{LOGO_URL}} /
// {{BASE_URL}} подставляются через strings.ReplaceAll (значения уже прошли
// html.EscapeString). НЕ использовать fmt — в CSS есть литералы '%'.
      ...
      <table role="presentation" class="pc-container" width="600" cellpadding="0" cellspacing="0" border="0" style="width:600px;max-width:600px;margin:0 auto;">
        <tr>
          <td align="center" style="padding:6px 0 26px 0;">
            <img src="{{LOGO_URL}}" width="150" alt="PROCLUSTER" style="display:block;width:150px;max-width:150px;height:auto;border:0;outline:none;text-decoration:none;">
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td align="center" style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;font-size:12px;line-height:1.7;color:#6b7280;padding:26px 16px 6px 16px;">
            © 2026 PROCLUSTER<br>
            <a href="{{BASE_URL}}" style="color:#9ca3af;text-decoration:underline;">chart.procluster.online</a>
          </td>
        </tr>
      </table>
    </td>
...
func buildVerificationMessage(from, to, verifyURL, baseURL string) []byte {
	...
	plain := "PROCLUSTER\r\n\r\n" +
		...
		"© 2026 PROCLUSTER · chart.procluster.online\r\n"

	base := strings.TrimRight(baseURL, "/")
	safeURL := html.EscapeString(verifyURL)
	safeBase := html.EscapeString(base)
	logoURL := html.EscapeString(base + "/logo.png")

	htmlBody := verifyEmailHTMLTemplate
	htmlBody = strings.ReplaceAll(htmlBody, "{{VERIFY_URL}}", safeURL)
	htmlBody = strings.ReplaceAll(htmlBody, "{{LOGO_URL}}", logoURL)
	htmlBody = strings.ReplaceAll(htmlBody, "{{BASE_URL}}", safeBase)
